...
class CircularCausalityEngine(eqx.Module):
    ...
    def _execute_core_processing(
        ...
        """Execute core circular causality processing."""
        # Self-reference processing
        self_referenced = self.self_reference_network(current_state)
        
        # Environment coupling
        coupling_input = jnp.concatenate([environmental_input, current_state])
        environment_coupled = self.environment_coupling_network(coupling_input)
    ...
    def _integrate_information_theory(
        ...
        """Integrate information theory analysis."""
        info_metrics = self._compute_information_theory_metrics(
            current_state, environmental_input, step_count
        )
        
        # Create enhanced state with information theory features
        info_features = jnp.array([
            info_metrics.get('circular_causality', 0.5),
            info_metrics.get('complexity_index', 0.5),
            info_metrics.get('entropy_rate', 0.3),
            info_metrics.get('coupling_coherence', 0.6),
            info_metrics.get('transfer_entropy_env_to_agent', 0.3),
            info_metrics.get('transfer_entropy_agent_to_env', 0.3),
            info_metrics.get('instantaneous_coupling', 0.4),
        ])
        
        # Enhance state using information integration network
        ...
        return info_enhanced_state, info_metrics
    ...
    def _process_dynamic_networks(
        ...
        """Process dynamic network connectivity."""
        # Compute network features
        network_features = self._compute_network_features(current_state, environmental_input)
        
        # Process through network state processor
        network_input = jnp.concatenate([current_state, network_features])
        network_enhanced_state = self.network_state_processor(network_input)
        
        # Update network connectivity based on current interaction
        # Pad environmental_input to match state dimension for proper connectivity update
        if environmental_input.shape[0] < current_state.shape[0]:
            padded_env_input = jnp.pad(
                environmental_input, 
                (0, current_state.shape[0] - environmental_input.shape[0]), 
                mode='constant'
            )
        else:
            padded_env_input = environmental_input[:current_state.shape[0]]
        
        connectivity_update = jnp.outer(current_state, padded_env_input) * 0.001
        updated_connectivity = self.network_connectivity + connectivity_update
        updated_connectivity = jnp.clip(updated_connectivity, -1.0, 1.0)
        ...
        return updated_self, network_enhanced_state, network_features
    ...
    def _integrate_circular_causality(
        ...
        """Integrate all processing stages into final circular causality."""
        ...
        emergent_meaning = self.meaning_emergence_network(combined_input)
        
        # Integrate for next state
        state_weights = self.circular_update_weights[:3]  # Use first 3 weights
        state_weights = state_weights / jnp.sum(state_weights)  # Normalize
        
        next_state = (
            state_weights[0] * processing_state['self_referenced'] +
            state_weights[1] * processing_state['environment_coupled'] +
            state_weights[2] * network_enhanced_state
        )
...
class AssociativeRecallSystem(eqx.Module):
    ...
    def __init__(
        ...
        self.emotional_encoder = eqx.nn.MLP(
            in_size=context_dim,  # Should match contextual_embedding dimension
    ...
    def recall_experiences(
        ...
        # Reshape query to be 2D: (1, embedding_dim)
        query_2d = query[None, :]  # (1, embedding_dim) 
        
        # Handle attention mechanism call - it might only return attended values
        attention_result = self.association_network(
            query_2d,                 # (1, embedding_dim)
            trace_matrix,             # (num_traces, embedding_dim)
            trace_matrix,             # (num_traces, embedding_dim)
        )
        
        # Extract attention scores using similarity as fallback
        if isinstance(attention_result, tuple) and len(attention_result) == 2:
            attended_recalls, attention_weights = attention_result
            attention_scores = attention_weights.squeeze()
        else:
            attended_recalls = attention_result
            # Compute similarity-based scores as fallback
            similarities = jnp.dot(trace_matrix, query) / (jnp.linalg.norm(trace_matrix, axis=1) * jnp.linalg.norm(query))
            attention_scores = jax.nn.softmax(similarities)
...
class IntegratedExperientialMemory(eqx.Module):
    ...
    def process_experiential_moment(
        ...
        updated_engine, next_state, emergent_meaning, causality_metrics = self.circular_engine.circular_causality_step(
            current_experience, environmental_input
        )
        
        # Update the circular engine with the returned updated version
        updated_self = eqx.tree_at(
            lambda x: x.circular_engine,
            self,
            updated_engine
        )
        ...
        max_traces = 100
        current_traces = updated_self.experience_traces
        if len(current_traces) >= max_traces:
            # Remove oldest trace
            current_traces = current_traces[1:]
        new_traces = current_traces + [new_trace]
        
        # Update traces immutably
        updated_self = eqx.tree_at(
            lambda x: x.experience_traces,
            updated_self,
            new_traces
        )
        
        # Sediment the experience immutably
        updated_sedimentation = updated_self.sedimentation.sediment_experience(
            next_state, significance_weight, contextual_cues
        )
        updated_self = eqx.tree_at(
            lambda x: x.sedimentation,
            updated_self,
            updated_sedimentation
        )
        
        # Recall similar experiences
        recalled_experiences = updated_self.recall_system.recall_experiences(
            current_experience, contextual_cues, updated_self.experience_traces
        )
        ...
        if recalled_experiences:
            ...
            if total_weight > 0:
                ...
                integrated_experience = updated_self.integration_network(integration_input)
            else:
        ...
        metadata = {
            ...
            'sedimentation_layers': updated_self.sedimentation.sediment_layers.shape[0],
            'emergent_meaning': emergent_meaning,
            'significance_weight': significance_weight,
            'trace_count': len(updated_self.experience_traces),
        }
